...
def process_items(li_regex, pbar, la_message):
    ...
    dict_data["timestamp"] = set()
    # set is used to speed up the process of checking if timestamp exists
    ...
    for i, item in enumerate(li_regex):
        ...
        if (ts not in dict_data["timestamp"]):
            dict_data["timestamp"].add(ts)
        
        dict_data[tag].append(val)

        pbar.setValue(int((i + 1) / len(li_regex) * 100))
    dict_data["timestamp"] = list(dict_data["timestamp"])
    return dict_data
